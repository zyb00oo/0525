...
class FeatureEncoder(nn.Module):
    def __init__(
        ...
        protein_resized = self._ensure_token_embedding_alignment(
            self.protein_model, self.protein_tokenizer
        )
        substrate_resized = self._ensure_token_embedding_alignment(
            self.substrate_model, self.substrate_tokenizer
        )
        ...
        if self.freeze_encoders:
            ...
            for p in self.substrate_model.parameters():
                p.requires_grad = False
            if protein_resized:
                self.protein_model.get_input_embeddings().weight.requires_grad = True
            if substrate_resized:
                self.substrate_model.get_input_embeddings().weight.requires_grad = True
    ...
    @staticmethod
    def _ensure_token_embedding_alignment(model: nn.Module, tokenizer) -> bool:
        token_count = len(tokenizer)
        emb = model.get_input_embeddings()
        if emb is None:
            return False
        current = emb.weight.size(0)
