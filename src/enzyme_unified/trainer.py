...
def _run_epoch(
    ...
    for step_idx, batch in enumerate(pbar, start=1):
        ...
        with torch.set_grad_enabled(train_mode):
            ...
            if "mol_graph_batch" in batch:
                feats["mol_graph_batch"] = batch["mol_graph_batch"].to(device)
            if "protein3d_batch" in batch:
                feats["protein3d_batch"] = batch["protein3d_batch"].to(device)
...
def train_and_evaluate_one_fold(
    ...
    for name, param in target_model.named_parameters():
        if not param.requires_grad:
            continue
        if any(key in name for key in gnn_param_keywords):
    ...
    if gnn_params:
        param_groups.append({"params": gnn_params, "lr": lr * gnn_lr_scale})
