...
def inverse_transform_targets(y_hat: np.ndarray, log_target: bool) -> np.ndarray:
    if log_target:
        ...
        # 避免 10**x 在极端预测下溢出为 inf，污染 raw 指标与相关系数计算
        y_hat_arr = np.clip(y_hat_arr, -50.0, 50.0)
        return np.power(10.0, y_hat_arr)
    return y_hat
...
def regression_metrics(y_true: Iterable[float], y_pred: Iterable[float]) -> Dict[str, float]:
    ...
    if not np.any(finite_mask):
        return {"rmse": float("nan"), "pcc": float("nan"), "scc": float("nan")}
